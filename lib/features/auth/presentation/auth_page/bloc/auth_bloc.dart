...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  AuthBloc(this._authRepository):
        super(const AuthState(status: BlocAuthStatus.initial, login: '', password: '')) {
  ...
  Future<void> _onAuthEventSubscriptionRequested(
      AuthEventSubscriptionRequested event,
      Emitter<AuthState> emit) async {
    await emit.forEach(
        _authRepository.getStatus(),
  ...
  Future<void> _onAuthEventLogIn(
    ...
    _authRepository.logIn(LoginCredentials(userIdentifier: state.login, password: state.password));
  }

  Future<void> _onAuthEventLoginChanged(
      AuthEventLoginChanged event,
      Emitter<AuthState> emit) async {
  ...
  Future<void> _onAuthEventPasswordChanged(
      AuthEventPasswordChanged event,
      Emitter<AuthState> emit) async {
